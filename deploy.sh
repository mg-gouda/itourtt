...
docker build --no-cache -t itourtt-backend:3.3.3 backend -q
...
if $needs_car_dispatch; then
  ...
  docker build --no-cache \
    ...
    -t itourtt-frontend:3.3.3-cardispatch frontend -q
  docker save itourtt-frontend:3.3.3-cardispatch | k3s ctr images import -
fi

if $needs_standard; then
  echo ">> Building frontend image (standard)..."
  docker build --no-cache -t itourtt-frontend:3.3.3 frontend -q
  docker save itourtt-frontend:3.3.3 | k3s ctr images import -
fi

# Import backend image into k3s
echo ">> Importing backend image into k3s..."
docker save itourtt-backend:3.3.3 | k3s ctr images import -
...
deploy_env() {
  ...
  kubectl set image deployment/backend backend=itourtt-backend:3.3.3 -n "$ns"
...
if [[ "$ENV" == "production" || "$ENV" == "all" ]]; then
  deploy_env "itour-production" "Production" "3.3.3-cardispatch"
fi

if [[ "$ENV" == "training" || "$ENV" == "all" ]]; then
  deploy_env "itour-training" "Training" "3.3.3-cardispatch"
fi

if [[ "$ENV" == "travelplan" || "$ENV" == "all" ]]; then
  deploy_env "itour-travelplan" "TravelPlan" "3.3.3"
fi
