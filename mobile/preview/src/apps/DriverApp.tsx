...
const MOCK_JOBS = [
  ...
  },
  {
    ...
    driverStatus: 'IN_PROGRESS' as const,
  ...
  },
  {
    ...
    driverStatus: 'COMPLETED' as const,
...
const MOCK_NOTIFICATIONS = [
  { id: '1', title: 'New Job Assigned', body: 'IT-2026-001847 - Airport pickup at 14:30', time: '2 min ago', read: false },
  { id: '2', title: 'Schedule Updated', body: 'Your 16:00 departure has been moved to 16:30', time: '15 min ago', read: false },
  { id: '3', title: 'Payment Received', body: 'Trip fee for IT-2026-001830 has been processed', time: '1 hour ago', read: true },
  { id: '4', title: 'Job Cancelled', body: 'IT-2026-001825 has been cancelled by dispatch', time: '3 hours ago', read: true },
  { id: '5', title: 'Daily Summary', body: 'You completed 6 trips yesterday. Total: 1,200 EGP', time: 'Yesterday', read: true },
];

const MOCK_HISTORY = [
  { ref: 'IT-2026-001830', route: 'Airport (T1) \u2192 Maadi Zone', status: 'COMPLETED', fee: '180 EGP', date: 'Feb 19' },
  { ref: 'IT-2026-001815', route: 'Heliopolis \u2192 Airport (T2)', status: 'COMPLETED', fee: '150 EGP', date: 'Feb 19' },
  { ref: 'IT-2026-001801', route: 'Airport (T3) \u2192 Zamalek', status: 'NO_SHOW', fee: '0 EGP', date: 'Feb 18' },
  { ref: 'IT-2026-001790', route: 'Giza \u2192 Cairo Airport (T1)', status: 'COMPLETED', fee: '200 EGP', date: 'Feb 18' },
  { ref: 'IT-2026-001778', route: 'Marriott \u2192 Citadel', status: 'COMPLETED', fee: '120 EGP', date: 'Feb 17' },
];
...
export function DriverApp({ dark }: Props) {
  ...
  const renderLogin = () => (
    <div className="login-screen">
      <div className="login-card">
        <div className="login-title">iTour Driver</div>
  ...
  const renderJobs = () => (
    <>
      <div className="screen-header">Today's Jobs</div>
      ...
      {MOCK_JOBS.map((job) => (
        <div
          key={job.id}
          className="job-card"
          style={{ borderLeftColor: getBorderColor(job.driverStatus), cursor: 'pointer' }}
          onClick={() => setScreen('detail')}
        >
          <div className="job-card-header">
            <div style={{ display: 'flex', gap: 6 }}>
              {getServiceBadge(job.serviceType)}
              {getStatusBadge(job.driverStatus)}
            </div>
          ...
          {job.driverStatus === 'PENDING' && (
            <div className="btn-row">
              <button className="btn btn-primary btn-sm" style={{ flex: 1 }}>Start Trip</button>
          ...
          {job.driverStatus === 'IN_PROGRESS' && (
            <div className="btn-row">
              <button className="btn btn-primary btn-sm" style={{ flex: 1 }}>Complete Trip</button>
              <button className="btn btn-destructive btn-sm" style={{ flex: 1 }} onClick={(e) => { e.stopPropagation(); setScreen('noshow'); }}>No Show</button>
            </div>
          )}
        </div>
      ...
      <>
        ...
        <div style={{ padding: '16px' }}>
          ...
          <div className="card" style={{ margin: 0, marginBottom: 12 }}>
            ...
            <div className="info-row"><span className="info-label">Vehicle</span><span className="info-value">Toyota Hiace - ABC 1234</span></div>
            <div className="info-row"><span className="info-label">Capacity</span><span className="info-value">10 pax</span></div>
          </div>
            ...
            <div className="card" style={{ margin: 0, marginBottom: 12 }}>
              ...
              <div className="collection-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>Amount Due</div>
                    <div style={{ fontSize: 18, fontWeight: 700 }}>{job.collectionAmount}</div>
                  </div>
                  <span className="badge badge-pending">Not Collected</span>
                </div>
              </div>
              <button className="btn btn-primary btn-sm" style={{ marginTop: 8 }}>Mark as Collected</button>
            </div>
          )}

          <div className="btn-row">
            <button className="btn btn-primary" style={{ flex: 1 }}>Start Trip</button>
    ...
    <>
      ...
      <div style={{ padding: 16 }}>
        ...
        <div className="card" style={{ margin: 0, marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: '#737373', marginBottom: 8 }}>Take photos of the location as proof</div>
        ...
        <div className="card" style={{ margin: 0, marginBottom: 16 }}>
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: '#22c55e' }}>
            30.0444, 31.2357 - Captured
          </div>
        </div>

        <div className="form-group">
          <label className="form-label">Notes (optional)</label>
          <input className="form-input" placeholder="Reason for no show..." />
        </div>
  ...
  const renderHistory = () => (
    <>
      <div className="screen-header">Trip History</div>
      ...
      <div className="card" style={{ background: dark ? '#171717' : '#f0fdf4', borderColor: dark ? '#262626' : '#bbf7d0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 24, fontWeight: 700, color: '#16a34a' }}>650 EGP</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 12, color: '#737373' }}>Trips</div>
    ...
    <>
      ...
      <div style={{ padding: 16 }}>
        <div style={{ textAlign: 'center', marginBottom: 20 }}>
          ...
          <div className="profile-name">Mohamed Khalil</div>
        ...
        <div className="card" style={{ margin: '0 0 12px' }}>
          ...
          <div className="profile-field">
            ...
            <div className="profile-field-value">+20 100 234 5678</div>
          </div>
          <div className="profile-field">
            <div className="profile-field-label">License Number</div>
            <div className="profile-field-value">DL-2024-78432</div>
          </div>
          <div className="profile-field">
            <div className="profile-field-label">License Expiry</div>
            <div className="profile-field-value">Dec 15, 2027</div>
          </div>
        </div>

        <div className="card" style={{ margin: '0 0 12px' }}>
          <div className="card-title">Assigned Vehicles</div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid ' + (dark ? '#262626' : '#f0f0f0') }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 500 }}>Toyota Hiace</div>
              <div style={{ fontSize: 12, color: '#737373' }}>ABC 1234 - 10 pax</div>
            </div>
            <span className="badge badge-completed">Primary</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0' }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 500 }}>Toyota Corolla</div>
              <div style={{ fontSize: 12, color: '#737373' }}>XYZ 5678 - 4 pax</div>
            </div>
            <span className="badge" style={{ background: '#f5f5f5', color: '#737373' }}>Backup</span>
          </div>
        </div>
