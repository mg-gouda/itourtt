...
**471 endpoints** across **40 controllers**. Global prefix `/api`.
...
- [ComplaintsController](#complaintscontroller) — 12 endpoints
...
| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/complaints` | `findAll`:53 | `perm:complaints.view` `perm:complaints` | `complaintsService.findAll` | GET /complaints — paginated list with status, stage, party, SLA and date filters. |
| POST | `/api/complaints` | `create`:77 | `perm:complaints.addButton` | `complaintsService.create` | POST /complaints — log a complaint against a job. |
| GET | `/api/complaints/:id` | `findOne`:68 | `perm:complaints.view` `perm:complaints` | `complaintsService.findOne` | GET /complaints/:id — one complaint with attachments, charge and adjustments. |
| PATCH | `/api/complaints/:id` | `update`:84 | `perm:complaints.editButton` | `complaintsService.update` | PATCH /complaints/:id — edit the complaint's details; amount fields need financial.editAmounts on top. |
| DELETE | `/api/complaints/:id` | `remove`:181 | `perm:complaints.deleteButton` | `complaintsService.remove` | DELETE /complaints/:id — soft-delete; refused once the charge has been posted. |
| PATCH | `/api/complaints/:id/assign` | `assign`:125 | `perm:complaints.assign` | `complaintsService.assign` | PATCH /complaints/:id/assign — set the dashboard user who owns the reply. |
| POST | `/api/complaints/:id/charge` | `createCharge`:142 | `perm:complaints.charge.create` | `chargesService.create` | POST /complaints/:id/charge — raise a deduction against the responsible party. |
| POST | `/api/complaints/:id/charge/approve` | `approveCharge`:153 | `perm:complaints.charge.approve` | `chargesService.approve` | POST /complaints/:id/charge/approve — agree to the deduction. |
| POST | `/api/complaints/:id/charge/post` | `postCharge`:164 | `perm:complaints.charge.post` | `chargesService.post` | POST /complaints/:id/charge/post — the only call that writes the negative fee row. |
| POST | `/api/complaints/:id/charge/void` | `voidCharge`:171 | `perm:complaints.charge.void` | `chargesService.void` | POST /complaints/:id/charge/void — reverse the deduction, compensating the fee row if it was already paid. |
| POST | `/api/complaints/:id/transition` | `transition`:106 | `perm:complaints.transition.review` `perm:complaints.transition.reply` `perm:complaints.transition.escalate` `perm:complaints.transition.resolve` `perm:complaints.transition.cancel` | `permissionsGuard.getUserPermissions` `complaintsService.transition` | POST /complaints/:id/transition — the only way status moves; checks the caller holds the key for that specific target state. |
| GET | `/api/complaints/job/:jobId` | `findByJob`:59 | `perm:complaints.view` `perm:complaints` | `complaintsService.findByJob` | GET /complaints/job/:jobId — every complaint on one job, for the job screen's Complaints tab. |
