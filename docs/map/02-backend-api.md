...
**475 endpoints** across **41 controllers**. Global prefix `/api`.
...
- [AgentAdjustmentsController](#agentadjustmentscontroller) — 5 endpoints
- [AgentsController](#agentscontroller) — 19 endpoints
...
- [ComplaintCategoriesController](#complaintcategoriescontroller) — 5 endpoints
- [ComplaintChargesController](#complaintchargescontroller) — 6 endpoints
- [ComplaintsController](#complaintscontroller) — 10 endpoints
...
- [RepPortalController](#repportalcontroller) — 16 endpoints
- [ReportsController](#reportscontroller) — 19 endpoints
...
`backend/src/complaints/agent-adjustments.controller.ts`

| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/agent-adjustments` | `findAll`:35 | `perm:finance.agentAdjustments.view` | `adjustmentsService.findAll` | GET /agent-adjustments — paginated list, filterable by status and agent. |
| GET | `/api/agent-adjustments/:id` | `findOne`:47 | `perm:finance.agentAdjustments.view` | `adjustmentsService.findOne` | GET /agent-adjustments/:id. |
| POST | `/api/agent-adjustments/:id/attach` | `attach`:53 | `perm:finance.agentAdjustments.onInvoice` | `adjustmentsService.attachToInvoice` | POST /agent-adjustments/:id/attach — put it on a draft invoice as a negative line. |
| POST | `/api/agent-adjustments/:id/disposition` | `disposition`:67 | `perm:finance.agentAdjustments.creditNote` `perm:finance.agentAdjustments.waive` | `permissionsGuard.getUserPermissions` `adjustmentsService.issueCreditNote` `adjustmentsService.waive` | POST /agent-adjustments/:id/disposition — issue a credit note or waive, each gated by its own key. |
| GET | `/api/agent-adjustments/agent/:agentId/pending` | `pendingForAgent`:41 | `perm:finance.agentAdjustments.view` | `adjustmentsService.pendingForAgent` | GET /agent-adjustments/agent/:agentId/pending — what is still owed to one agent. |
...
| POST | `/api/agents` | `create`:110 | `ADMIN` `AGENT_MANAGER` `perm:agents.addButton` | `agentsService.create` | Creates an agent. |
| GET | `/api/agents/:id` | `findOne`:117 | `perm:agents` | `agentsService.findOne` | One agent's full profile. |
| PUT | `/api/agents/:id` | `update`:125 | `ADMIN` `AGENT_MANAGER` `perm:agents.table.editButton` | `agentsService.update` | Edits an agent, including the `refPattern` used to validate job references. |
| DELETE | `/api/agents/:id` | `delete`:152 | `ADMIN` `perm:agents.table.deleteButton` | `agentsService.delete` | Soft-deletes an agent. |
| PUT | `/api/agents/:id/credit` | `updateCredit`:167 | `ADMIN` `AGENT_MANAGER` `perm:agents.table.editButton` | `agentsService.updateCredit` | Sets credit limit, days and currency. |
| GET | `/api/agents/:id/credit-status` | `getCreditStatus`:159 | `perm:agents` | `agentsService.getCreditStatus` | Credit limit versus outstanding invoices. |
| POST | `/api/agents/:id/documents` | `createDocument`:189 | `ADMIN` `AGENT_MANAGER` `perm:agents.table.editButton` | `agentsService.createDocument` | Uploads a legal document for the agent. |
| GET | `/api/agents/:id/documents` | `findDocuments`:199 | `perm:agents` | `agentsService.findDocuments` | Lists the agent's legal documents. |
| PUT | `/api/agents/:id/invoice-cycle` | `updateInvoiceCycle`:178 | `ADMIN` `AGENT_MANAGER` `perm:agents.table.editButton` | `agentsService.updateInvoiceCycle` | Sets the invoicing cadence. |
| GET | `/api/agents/:id/price-list` | `getPriceList`:208 | `perm:agents` | `agentsService.getPriceList` | The agent's price grid. |
| POST | `/api/agents/:id/price-list` | `upsertPriceList`:216 | `ADMIN` `AGENT_MANAGER` `perm:agents.table.editButton` | `agentsService.upsertPriceItems` | Bulk-saves the agent price grid. |
| DELETE | `/api/agents/:id/price-list/:priceItemId` | `deletePriceItem`:227 | `ADMIN` `AGENT_MANAGER` `perm:agents.table.editButton` | `agentsService.deletePriceItem` | Removes one price row. |
| PATCH | `/api/agents/:id/status` | `toggleStatus`:136 | `ADMIN` `perm:agents.table.toggleStatus` | `agentsService.toggleStatus` | Activates/deactivates an agent. |
| DELETE | `/api/agents/bulk` | `bulkDelete`:144 | `ADMIN` `perm:agents.table.deleteButton` | `agentsService.bulkDelete` | Soft-deletes several agents. |
| GET | `/api/agents/export/excel` | `exportExcel`:68 | `ADMIN` `AGENT_MANAGER` `perm:agents.export` | `agentsService.exportToExcel` | Downloads agents as xlsx. |
| POST | `/api/agents/import/excel` | `importExcel`:96 | `ADMIN` `AGENT_MANAGER` `perm:agents.import` | `agentsService.importFromExcel` | Multipart bulk agent import. |
| GET | `/api/agents/import/template` | `downloadTemplate`:82 | `ADMIN` `AGENT_MANAGER` `perm:agents.downloadTemplate` | `agentsService.generateImportTemplate` | Downloads the blank agent import workbook. |
| GET | `/api/agents/lookup` | `lookup`:61 | `perm:agents` `perm:complaints.addButton` `perm:complaints.editButton` `perm:complaints.view` | `agentsService.lookup` | GET /agents/lookup — name-only agent list for pickers, readable by complaint loggers without exposing the full agent profile. |
...
`backend/src/complaints/complaint-categories.controller.ts`
...
`backend/src/complaints/complaint-charges.controller.ts`

| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/complaint-charges` | `findByJob`:42 | `perm:dispatch.deductionButton` `perm:complaints.charge.create` | `chargesService.findByJob` | Every deduction standing on one job. |
| POST | `/api/complaint-charges` | `create`:48 | `perm:dispatch.deductionButton` `perm:complaints.charge.create` | `chargesService.create` | Raises a deduction on a job, with or without a complaint. |
| PATCH | `/api/complaint-charges/:id` | `update`:59 | `perm:dispatch.deductionButton` `perm:complaints.charge.create` | `chargesService.update` | Overrides a pending deduction's amount, currency or reason. |
| POST | `/api/complaint-charges/:id/approve` | `approve`:69 | `perm:complaints.charge.approve` | `chargesService.approve` | Agrees to a pending deduction. |
| POST | `/api/complaint-charges/:id/post` | `post`:80 | `perm:complaints.charge.post` | `chargesService.post` | Writes the deduction into the party's fee table — the only call that moves money. |
| POST | `/api/complaint-charges/:id/void` | `void`:87 | `perm:complaints.charge.void` | `chargesService.void` | Reverses a deduction, refunding it if it was already paid out. |
...
| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/complaints` | `findAll`:50 | `perm:complaints.view` `perm:complaints` | `complaintsService.findAll` | GET /complaints — paginated list with status, stage, party, SLA and date filters. |
| POST | `/api/complaints` | `create`:100 | `perm:complaints.addButton` | `complaintsService.create` | POST /complaints — log a complaint against a job. |
| GET | `/api/complaints/:id` | `findOne`:91 | `perm:complaints.view` `perm:complaints` | `complaintsService.findOne` | GET /complaints/:id — one complaint with attachments, charge and adjustments. |
| PATCH | `/api/complaints/:id` | `update`:107 | `perm:complaints.editButton` | `complaintsService.update` | PATCH /complaints/:id — edit the complaint's details; amount fields need financial.editAmounts on top. |
| DELETE | `/api/complaints/:id` | `remove`:159 | `perm:complaints.deleteButton` | `complaintsService.remove` | DELETE /complaints/:id — soft-delete; refused once the charge has been posted. |
| PATCH | `/api/complaints/:id/assign` | `assign`:149 | `perm:complaints.assign` | `complaintsService.assign` | PATCH /complaints/:id/assign — set the dashboard user who owns the reply. |
| POST | `/api/complaints/:id/transition` | `transition`:130 | `perm:complaints.transition.review` `perm:complaints.transition.reply` `perm:complaints.transition.escalate` `perm:complaints.transition.resolve` `perm:complaints.transition.cancel` | `permissionsGuard.getUserPermissions` `complaintsService.transition` | POST /complaints/:id/transition — the only way status moves; checks the caller holds the key for that specific target state. |
| GET | `/api/complaints/analytics` | `analytics`:61 | `perm:complaints.analytics` `perm:complaints` | `complaintsService.canViewAmounts` `analyticsService.summary` | GET /complaints/analytics — the analytics summary for a filtered range; declared before :id so the word is never read as a complaint id. |
| GET | `/api/complaints/assignable-users` | `assignableUsers`:76 | `perm:complaints.assign` `perm:complaints.addButton` `perm:complaints.editButton` | `complaintsService.assignableUsers` | Staff a complaint can be handed to — the owner picker's source, unlike ADMIN-only GET /users. |
| GET | `/api/complaints/job/:jobId` | `findByJob`:82 | `perm:complaints.view` `perm:complaints` | `complaintsService.findByJob` | GET /complaints/job/:jobId — every complaint on one job, for the job screen's Complaints tab. |
...
| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/driver-portal/complaints` | `getComplaints`:218 | `DRIVER` | `driverPortalService.getComplaints` | GET /driver-portal/complaints — the driver's own settled complaints. |
...
| GET | `/api/driver-portal/notifications` | `getNotifications`:224 | `DRIVER` | `driverPortalService.getNotifications` | Driver's notification feed. |
| PATCH | `/api/driver-portal/notifications/:id/read` | `markNotificationRead`:230 | `DRIVER` | `driverPortalService.markNotificationRead` | Marks a single notification read. |
| PATCH | `/api/driver-portal/notifications/read-all` | `markAllRead`:239 | `DRIVER` | `driverPortalService.markAllRead` | Marks every notification read for this driver. |
| GET | `/api/driver-portal/profile` | `getProfile`:245 | `DRIVER` | `driverPortalService.getProfile` | The driver's own profile. |
...
| GET | `/api/export/odoo/car-jobs-excel` | `exportCarJobsExcel`:505 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.carJobs` | `exportService.exportCarJobsExcel` | Car jobs (owned fleet utilisation) as xlsx. |
...
| GET | `/api/export/odoo/complaints` | `exportComplaints`:414 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.complaints` | `exportService.exportComplaintsReport` | GET /export/odoo/complaints — downloads the complaints report as xlsx. |
...
| GET | `/api/export/odoo/departure` | `exportDeparture`:465 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.departure` | `exportService.exportDepartureReport` | Departures report as xlsx. |
...
| GET | `/api/export/odoo/flight-delay` | `exportFlightDelay`:478 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.flightDelay` | `exportService.exportFlightDelayReport` | Flight delay report as xlsx. |
| GET | `/api/export/odoo/guest-surveys` | `exportGuestSurveys`:435 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.guestSurveys` | `exportService.exportGuestSurveyReport` | Arrival guest surveys as xlsx (reuses the `reports.guestSurveys` permission). |
...
| GET | `/api/export/odoo/job-status` | `exportJobStatus`:448 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.jobStatus` | `exportService.exportJobStatusReport` | Job status breakdown as xlsx. |
...
| GET | `/api/export/odoo/review` | `exportReview`:518 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.review` | `exportService.exportReviewReport` | Guest review report as xlsx. |
...
| GET | `/api/export/odoo/supplier-jobs-excel` | `exportSupplierJobsExcel`:491 | `ADMIN` `MANAGER` `ACCOUNTANT` `perm:reports.supplierJobs` | `exportService.exportSupplierJobsExcel` | Supplier jobs report as xlsx. |
...
| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/rep-portal/complaints` | `getComplaints`:288 | `REP` | `repPortalService.getComplaints` | GET /rep-portal/complaints — the rep's own settled complaints. |
...
| GET | `/api/rep-portal/notifications` | `getNotifications`:294 | `REP` | `repPortalService.getNotifications` | Rep's notification feed. |
| PATCH | `/api/rep-portal/notifications/:id/read` | `markNotificationRead`:300 | `REP` | `repPortalService.markNotificationRead` | Marks a single notification read. |
| PATCH | `/api/rep-portal/notifications/read-all` | `markAllRead`:309 | `REP` | `repPortalService.markAllRead` | Marks every notification read for this rep. |
| GET | `/api/rep-portal/profile` | `getProfile`:315 | `REP` | `repPortalService.getProfile` | The rep's own profile. |
...
| Method | Path | Handler | Auth | Calls | Purpose |
|---|---|---|---|---|---|
| GET | `/api/reports/agent-statement/:agentId` | `agentStatement`:236 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.agentStatement` | `reportsService.agentStatement` | Statement of account for an agent: invoices, payments and balance. |
| GET | `/api/reports/complaints` | `complaintsReport`:324 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.complaints` | `reportsService.complaintsReport` | GET /reports/complaints — the complaints report, filterable by status, agent, category and responsible party. |
| GET | `/api/reports/daily-dispatch` | `dailyDispatchSummary`:201 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.dailyDispatch` | `reportsService.dailyDispatchSummary` | One day's dispatch counts and status breakdown. |
| GET | `/api/reports/departure` | `departureReport`:390 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.departure` | `reportsService.departureReport` | Departure jobs report. |
| GET | `/api/reports/driver-score` | `driverScoreReport`:339 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.driverScore` | `reportsService.driverScoreReport` | Driver scores across jobs. |
| PUT | `/api/reports/driver-score/:jobId` | `upsertDriverScore`:285 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.driverScore` | `reportsService.upsertDriverScore` | Sets a driver's per-job score, recalculating the fee it implies. |
| GET | `/api/reports/driver-trips` | `driverTripReport`:221 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.driverTrips` | `reportsService.driverTripReport` | Trips and fees per driver over a period. |
| GET | `/api/reports/evidence` | `evidenceReport`:353 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.evidence` | `reportsService.evidenceReport` | Which jobs have evidence photos and which are missing them. |
| GET | `/api/reports/flight-delay` | `flightDelayReport`:424 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.flightDelay` | `reportsService.flightDelayReport` | Flight delays reported by reps, reconstructed from the notifications they raised. |
| GET | `/api/reports/flight-delay/:jobId` | `flightDelayForJob`:438 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.flightDelay` | `reportsService.flightDelayForJob` | Delay history for one job. |
| GET | `/api/reports/guest-surveys` | `guestSurveyReport`:310 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.guestSurveys` | `reportsService.guestSurveyReport` | Arrival guest survey responses. |
| GET | `/api/reports/job-status` | `jobStatusReport`:404 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.jobStatus` | `reportsService.jobStatusReport` | Job counts by status over a period. |
| GET | `/api/reports/rep-fees` | `repFeeReport`:211 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.repFees` | `reportsService.repFeeReport` | Rep fees per job, with the score behind each. |
| GET | `/api/reports/rep-score` | `repScoreReport`:296 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.repScore` | `reportsService.repScoreReport` | Rep scores across jobs. |
| PUT | `/api/reports/rep-score/:jobId` | `upsertRepScore`:270 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.repScore` | `reportsService.upsertRepScore` | Sets a rep's per-job score flags and recalculates the fee via `scoreToFeeAndEval` — this is the write path behind the Rep Fees modal. |
| GET | `/api/reports/revenue` | `revenueReport`:255 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.revenue` | `reportsService.revenueReport` | Revenue against driver, rep and supplier costs for a period. |
| GET | `/api/reports/review` | `reviewReport`:445 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.review` | `reportsService.reviewReport` | Guest reviews per job. |
| GET | `/api/reports/sales` | `salesReport`:380 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.sales` | `reportsService.salesReport` | Sales by period and counterparty. |
| GET | `/api/reports/visa` | `visaReport`:370 | `ADMIN` `MANAGER` `ACCOUNTANT` `DISPATCHER` `perm:reports.visa` | `reportsService.visaReport` | Visa-related job report. |
